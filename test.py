...
img_hist = cv2.calcHist(img_new, [0, 1, 2], None, [8, 8, 8],[0, 256, 0, 256, 0, 256])
cv2.normalize(src=img_hist, dst=img_hist).flatten()
img2_hist = cv2.calcHist(img2_new, [0, 1, 2], None, [8, 8, 8],[0, 256, 0, 256, 0, 256])
cv2.normalize(src=img2_hist, dst=img2_hist).flatten()

histogram = cv2.compareHist(img_hist, img2_hist, cv2.HISTCMP_HELLINGER)
